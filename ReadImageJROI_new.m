function [sROI] = ReadImageJROI_new(cstrFilenames)
...
% this has been changed to use java engine zip reader
...
if (isequal(lower(strExt), '.zip'))
   ...
   strROIDir = tempname;
   cstrFilenames_bla = unzip(strFilename, strROIDir);
   
   for i=1:length(cstrFilenames_short)
       cstrFilenames(1,i) = {[strROIDir '/' char(cstrFilenames_short(i,1)) ]};
   end;
   ...
   cvsROIs = ReadImageJROI_new(cstrFilenames);
