...
function out=dftups_off(in,nor,noc,usfac,roff,coff)
...
function out=dftups_nooff(in,nor,noc,usfac)
% function out=dftups(in,nor,noc,usfac);
% Upsampled DFT by matrix multiplies, can compute an upsampled DFT in just
% a small region.
% usfac         Upsampling factor (default usfac = 1)
% [nor,noc]     Number of pixels in the output upsampled DFT, in
...
[nr,nc]=size(in);
% Compute kernels and obtain DFT by matrix products
kernc=exp((-1i*2*pi/(nc*usfac))*( ifftshift(0:nc-1).' - floor(nc/2) )*( 0:noc-1));
kernr=exp((-1i*2*pi/(nr*usfac))*( (0:nor-1).')*( ifftshift(0:nr-1) - floor(nr/2)  ));
out=kernr*in*kernc;
return
